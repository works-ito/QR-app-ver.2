...
if start < 0 or end < 0:
    raise SystemExit("analyzeWizardSlipPhoto block not found")

if text.count(start_marker) != 1:
    raise SystemExit(
        f"unexpected analyzeWizardSlipPhoto count: {text.count(start_marker)}"
    )
...
loader = '    .then(function() { return loadScript("./gemini-whole-image-dev.js?v=80"); })\n'

if loader not in sales_text:
    raise SystemExit("gemini-whole-image-dev loader not found")

sales_text = sales_text.replace(loader, "", 1)
sales_text = sales_text.replace(
    "/* 販売品入庫受付 v147 bootstrap */",
    "/* 販売品入庫受付 v148 bootstrap */",
    1,
)
...
if "./app.js?v=76" not in index_text:
    raise SystemExit("app.js v76 reference not found")

if "./sales-stockin.js?v=147" not in index_text:
    raise SystemExit("sales-stockin.js v147 reference not found")

index_text = index_text.replace("./app.js?v=76", "./app.js?v=77", 1)
index_text = index_text.replace(
    "./sales-stockin.js?v=147",
    "./sales-stockin.js?v=148",
    1,
)
...
Path("version.json").write_text(
    json.dumps(
        {"version": "2.2-20260829-ai-single1"},
        ensure_ascii=False,
    )
    + "\n",
    encoding="utf-8",
...
print("AI refactor patch completed")
